...
if ( ! class_exists( 'WFACP_Compatibility_Facturare_WC' ) ) {
	...
	class WFACP_Compatibility_Facturare_WC {
		private $obj           = null;
		private $px            = 7;
		private $facturare_arr = array( 'tip_facturare', 'cnp', 'cui', 'nr_reg_com', 'nume_banca', 'iban', 'billing_company' );

		public function __construct() {

			/* Register Add field */            add_action( 'wfacp_after_checkout_page_found', array( $this, 'actions' ) );
		...
		public function add_field( $fields ) {
			...
			$fields['wfacp_tip_facturare'] = array(
				'type'       => 'wfacp_html',
				'class'      => array( 'wfacp-col-full', 'wfacp-form-control-wrapper', 'wfacp_anim_wrap', 'WooFacturare' ),
				'id'         => 'wfacp_tip_facturare',
				'field_type' => 'wfacp_tip_facturare',
				'label'      => __( 'WooFacturare', 'woofunnels-aero-checkout' ),

			);
			$fields['tip_facturare_fields'] = array(
				'type'       => 'wfacp_html',
				'class'      => array( 'wfacp-col-full', 'wfacp-form-control-wrapper', 'wfacp_anim_wrap', 'WooFacturare_fields' ),
		...
		public function process_wfacp_html( $field, $key ) {
			...
			if ( isset( $checkoutFields['billing']['billing_company'] ) ) {
				...
				if ( is_array( $billing_company ) && count( $billing_company ) > 0 ) {
					$checkoutFields['billing_company'] = array(
						'label'         => 'Company',
			...
			if ( isset( $checkoutFields['billing']['billing_last_name'] ) ) {
				...
				if ( is_array( $billing_last_name ) && count( $billing_last_name ) > 0 ) {
					$checkoutFields['billing_last_name'] = array(
						'label'        => 'Last name',
						'required'     => 'true',
		...
		public function add_default_wfacp_styling( $args, $key ) {
			...
			if ( ! in_array( 'av_tip_facturare_radio', $args['class'] ) ) {
				...
				try {
					$args['class']       = array_merge( array( 'wfacp-form-control-wrapper', $width_class ), $args['class'] );
					$args['cssready']    = array( $width_class );
