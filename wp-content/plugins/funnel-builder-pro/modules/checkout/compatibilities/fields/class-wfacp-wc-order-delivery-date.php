...
if ( ! class_exists( 'WFACP_Compatibility_WC_delivery_date' ) ) {
	...
	class WFACP_Compatibility_WC_delivery_date {
		...
		public function __construct() {
			...
			/* Register Add field */
			add_action( 'wfacp_template_load', array( $this, 'remove_wc_delivery_date_hook' ) );
			add_filter( 'wfacp_advanced_fields', array( $this, 'add_field' ), 20 );

			add_filter( 'wfacp_html_fields_wfacp_delivery_date', '__return_false' );

			add_action( 'process_wfacp_html', array( $this, 'call_wc_delivery_date_hook' ), 10, 3 );
		...
		public function add_field( $fields ) {
			...
			$fields['wfacp_delivery_date'] = array(
				'type'       => 'wfacp_html',
				'class'      => array( 'wfacp-col-full', 'wfacp-form-control-wrapper', 'wfacp_anim_wrap' ),
		...
		public function add_default_wfacp_styling( $args, $key ) {
			...
			if ( $key == 'delivery_date' && $this->is_enable() ) {
				$args['input_class'] = array( 'wfacp-form-control' );
			}
