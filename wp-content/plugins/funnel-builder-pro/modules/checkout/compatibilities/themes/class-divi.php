...
if ( ! class_exists( 'WFACP_Compatibility_With_Divi' ) ) {
	...
	class WFACP_Compatibility_With_Divi {
		...
		public function __construct() {
			...
			/* Remove Divi's WooCommerce dynamic CSS on checkout pages to prevent style conflicts */
			add_filter( 'divi_frontend_assets_dynamic_assets_global_assets_list', array( $this, 'remove_divi_woocommerce_css' ), 20 );
		...
		/**
		 * Remove Divi's WooCommerce dynamic CSS assets on checkout pages.
		 * Prevents woocommerce.css and woocommerce_shared.css from Divi conflicting with checkout form styles.
		 *
		 * @param array $assets_list Divi dynamic assets list.
		 * @return array Filtered assets list.
		 */
		public function remove_divi_woocommerce_css( $assets_list ) {
			if ( ! WFACP_Common::get_id() ) {
				return $assets_list;
			}

			unset( $assets_list['et_divi_woocommerce'] );
			unset( $assets_list['et_divi_woocommerce_modules'] );
