...
if ( ! class_exists( 'WFACP_Checkout_addons' ) ) {
	...
	class WFACP_Checkout_addons {
		...
		public function __construct() {
			...
			add_action( 'process_wfacp_html', array( $this, 'call_checkout_add_on' ), 10, 3 );
			...
			add_action( 'wfacp_after_template_found', array( $this, 'after_template_load' ) );
			add_filter( 'wfacp_checkout_fields', array( $this, 'remove_addons_field' ) );
			add_filter( 'woocommerce_update_order_review_fragments', array( $this, 'refresh_checkout_fields_frag' ), 11 );
			add_filter( 'wp_footer', array( $this, 'add_js' ), 11 );
		...
		public function after_template_load() {
			add_action( 'woocommerce_before_checkout_form', array( $this, 'actions' ) );
		...
		public function add_fields( $field ) {
			...
			$field['wc_checkout_add_on'] = array(
				'type'       => 'wfacp_html',
				'class'      => array( 'wc_checkout_add_on' ),
		...
		private function render_add_ons() {
			...
			if ( is_array( $checkout_add_on_fields ) && count( $checkout_add_on_fields ) > 0 ) {
				foreach ( $checkout_add_on_fields as $key => $field ) {
					...
					if ( $type == 'select' ) {
						$field['class'] = array( 'wfacp_custom_wrap' );
					}

					if ( $type == 'text' || $type == 'select' || $type == 'textarea' ) {
						$class = array( 'wfacp_checkout_addon_wrap' );
						...
						$field['class'] = array( 'wfacp_default_checkout_addon_multicheckbox', 'wfacp_checkout_addon_wrap' );
					} elseif ( $type == 'wc_checkout_add_ons_file' ) {
						$class          = array( 'wc_checkout_add_ons_fileupload', 'wfacp_checkout_addon_wrap' );
						$field['class'] = $class;
					} elseif ( $type == 'wc_checkout_add_ons_multiselect' ) {
						$field['class'] = array( 'wc_checkout_add_ons_multiselect', 'wfacp_checkout_addon_wrap' );
					} elseif ( $type == 'wc_checkout_add_ons_radio' ) {
						$field['class'] = array( 'wc_checkout_add_ons_radio', 'wfacp_checkout_addon_wrap' );
					}
