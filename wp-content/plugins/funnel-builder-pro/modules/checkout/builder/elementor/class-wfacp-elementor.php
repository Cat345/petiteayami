...
if ( ! class_exists( 'WFACP_Elementor' ) ) {
	...
	class WFACP_Elementor {
		...
		public static function get_widget_names() {
			$names = ! empty( self::$widget_names ) ? self::$widget_names : array( 'wfacp_form', 'wfacp_form_summary' );

			return apply_filters( 'wfacp_elementor_widget_names', $names );
		}

		/**
		 * Hide our widgets from the Elementor panel (and its search) on every document
		 * that is not a FunnelKit Elementor checkout.
		 *
		 ...
		 * editor's shouldAddWidget() only tests `show_in_panel`, so a widget whose category
		 * is absent still lands in the elements collection and stays reachable through the
		 * panel search box. `hide_on_search` exists precisely for that case.
		 *
		 * This mirrors how Elementor Pro scopes its own widgets
		 * (see ElementorPro\Modules\Woocommerce\Module::hide_woocommerce_widgets_in_loop_document).
		 *
		 * @param array $config
		 * @param int   $post_id
		 *
		 ...
		 * (see the comment in register()). Changing the registration only affects future
		 * cache builds - a row that is already in postmeta keeps being served until its TTL
		 * expires, which can be up to a year. This clears them once on upgrade.
		 *
		 * Deliberately admin-only and version-gated: nothing runs on front-end requests.
		 *
		 * @return void
		 */
		public function maybe_flush_elementor_element_cache() {
			 ...
			 * delete_post_meta(), so nothing invalidates the 'post_meta' object cache for
			 * those posts. On a site running a persistent object cache the row would be gone
			 * while get_post_meta() kept serving the stale (poisoned) value - precisely on the
			 * high-traffic installs this routine exists for. One indexed SELECT, then one DELETE.
			 */
		 ...
		 * our widgets register no controls and the generated file contains none of the
		...
		private function register() {
			 ...
			 *
			 * Registering site-wide keeps the widget in the tree everywhere. Rendering stays
			 * context-guarded inside the widget itself (see WFACP_Elementor_HTML_BLOCK::render()),
			 * and `is_dynamic_content()` guarantees Elementor stores a re-rendered placeholder
			 * rather than baking our markup into the cache.
			 *
			 * Panel visibility is scoped separately - see hide_widgets_outside_checkout().
			 */
			$this->widgets();
