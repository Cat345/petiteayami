...
if ( ! class_exists( 'WFACP_Template_Divi5' ) ) {
	...
	protected function __construct() {
		...
		remove_filter( 'wfacp_forms_field', [ $this, 'modern_label' ], 20 );
		remove_action( 'wfacp_before_mini_cart_html', [ $this, 'display_mini_cart_undo_message' ] );
		remove_action( 'wfacp_before_order_summary', [ $this, 'display_order_summary_undo_message' ] );
		remove_action( 'wfacp_before_product_switcher_html', [ $this, 'display_undo_message' ] );
		remove_filter( 'woocommerce_update_order_review_fragments', [ $this, 'add_checkout_fragments' ], 99 );
		remove_filter( 'woocommerce_order_button_html', [ $this, 'add_class_change_place_order' ], 11 );
		remove_action( 'wfacp_form_single_step_start', [ $this, 'preview_field_generate' ] );
		remove_action( 'wfacp_form_two_step_start', [ $this, 'preview_field_generate' ] );
		remove_action( 'wfacp_form_third_step_start', [ $this, 'preview_field_generate' ] );
	}
		...
		if ( isset( $data['exchange_keys'] ) && isset( $data['exchange_keys']['divi'] ) ) {
			...
			foreach ( $divi_keys as $key => $value ) {
				if ( is_string( $value ) ) {
					...
					$sanitized_keys[ $key ] = str_replace( [ '/', '\\', ' ', '.', ':', '[', ']', '(', ')', '{', '}' ], '_', $value );
				} else {
