...
trait RestApiTrait {
	...
	public static function rest_get_field_structure( \WP_REST_Request $request ) {
		...
		if ( $post_id > 0 ) {
			...
			$wfacp_post_id = absint( wp_unslash( $_REQUEST['post_id'] ) );
		} elseif ( isset( $_REQUEST['et_post_id'] ) ) {
			$wfacp_post_id = absint( wp_unslash( $_REQUEST['et_post_id'] ) );
		} elseif ( isset( $_REQUEST['et_wfacp_id'] ) ) {
			$wfacp_post_id = absint( wp_unslash( $_REQUEST['et_wfacp_id'] ) );
		}
		...
		if ( $wfacp_post_id === 0 ) {
			...
			if ( function_exists( 'et_fb_is_enabled' ) && et_fb_is_enabled() ) {
				...
				$is_visual_builder = true;
			} elseif ( isset( $_GET['et_fb'] ) && sanitize_text_field( wp_unslash( $_GET['et_fb'] ) ) === '1' ) {
				$is_visual_builder = true;
			}
