...
trait RenderCallbackTrait {
	...
	public static function render_callback( array $attrs, string $content, \WP_Block $block, $elements, array $default_printed_style_attrs = [] ): string {
		try {
			...
			$form_data['collapsible_mini_cart_enable_strike_through_price'] = $attrs['collapsible_mini_cart_enable_strike_through_price']['desktop']['value'] ?? 'off';

			// Low stock trigger
			$form_data['collapsible_mini_cart_enable_low_stock_trigger'] = $attrs['collapsible_mini_cart_enable_low_stock_trigger']['desktop']['value'] ?? 'off';
			if ( isset( $attrs['collapsible_mini_cart_low_stock_message']['innerContent']['desktop']['value'] ) ) {
				$form_data['collapsible_mini_cart_low_stock_message'] = $attrs['collapsible_mini_cart_low_stock_message']['innerContent']['desktop']['value'];
			}

			// Saving price message
			$form_data['collapsible_mini_cart_enable_saving_price_message'] = $attrs['collapsible_mini_cart_enable_saving_price_message']['desktop']['value'] ?? 'off';
			if ( isset( $attrs['collapsible_mini_cart_saving_price_message']['innerContent']['desktop']['value'] ) ) {
				$form_data['collapsible_mini_cart_saving_price_message'] = $attrs['collapsible_mini_cart_saving_price_message']['innerContent']['desktop']['value'];
			}
			...
			// Strike through price
			$form_data['order_summary_field_enable_strike_through_price'] = $attrs['order_summary_field_enable_strike_through_price']['desktop']['value'] ?? 'off';

			// Low stock trigger
			$form_data['order_summary_field_enable_low_stock_trigger'] = $attrs['order_summary_field_enable_low_stock_trigger']['desktop']['value'] ?? 'off';
			if ( isset( $attrs['order_summary_field_low_stock_message']['innerContent']['desktop']['value'] ) ) {
				$form_data['order_summary_field_low_stock_message'] = $attrs['order_summary_field_low_stock_message']['innerContent']['desktop']['value'];
			}

			// Saving price message
			$form_data['order_summary_field_enable_saving_price_message'] = $attrs['order_summary_field_enable_saving_price_message']['desktop']['value'] ?? 'off';
			if ( isset( $attrs['order_summary_field_saving_price_message']['innerContent']['desktop']['value'] ) ) {
				$form_data['order_summary_field_saving_price_message'] = $attrs['order_summary_field_saving_price_message']['innerContent']['desktop']['value'];
			}
			...
			if ( empty( $form_data['wfacp_payment_place_order_text'] ) ) {
				$form_data['wfacp_payment_place_order_text'] = __( 'Place order', 'woocommerce' );
			}
			...
			if ( isset( $attrs['form_coupon_button_text']['innerContent']['desktop']['value'] ) ) {
				...
				$form_data['form_coupon_button_text'] = __( 'Apply', 'woocommerce' );
			}
			...
			$rendered_output = Module::render(
				[
					...
					'classnamesFunction'  => [ CheckoutForm::class, 'module_classnames' ],
					'stylesComponent'     => [ CheckoutForm::class, 'module_styles' ],
					'scriptDataComponent' => [ CheckoutForm::class, 'module_script_data' ],
					'parentAttrs'         => [],
					'parentId'            => '',
					'parentName'          => '',
					'children'            => [
