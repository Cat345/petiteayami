...
if ( ! defined( 'WFACP_DIVI5_MODULES_LOADED' ) ) {
	...
	add_action(
		...
		function () {
			...
			// MiniCart REST routes.
			$mc_file = $modules_dir . 'MiniCart/MiniCart.php';
			if ( file_exists( $mc_file ) ) {
				require_once $mc_file;
				if ( class_exists( 'WFACP\\Modules\\MiniCart\\MiniCart' ) ) {
					\WFACP\Modules\MiniCart\MiniCart::register_rest_routes_direct();
				}
			}
		}
	);
...
function disable_photoswipe_in_divi5_vb( $enable ) {
	...
	elseif ( ( isset( $_REQUEST['et_fb'] ) && sanitize_text_field( wp_unslash( $_REQUEST['et_fb'] ) ) === '1' ) ||
	         ( isset( $_REQUEST['et_wfacp_id'] ) && ! empty( sanitize_text_field( wp_unslash( $_REQUEST['et_wfacp_id'] ) ) ) ) ) {
		$is_vb = true;
	}
 ...
 */
function convert_border_width( $value, $context = [] ) {
	if ( is_numeric( $value ) ) {
		return $value . 'px';
	}
	return $value;
