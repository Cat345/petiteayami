...
if ( ! class_exists( 'WFACP_Common_Helper' ) ) {
	...
	abstract class WFACP_Common_Helper {
		 ...
		 * @return array
		 */
		final public static function get_cart_item_attributes( $cart_item, $pro, $product_data, $cart_variation_id ) {
			...
			if ( count( $product_attributes ) > 0 ) {
				...
				foreach ( $product_attributes as $a_key => $a_val ) {
					...
					} elseif ( in_array( $product_data['type'], WFACP_Common::get_variable_product_type() ) && $cart_variation_id > 0 ) {
							$is_variation_error = 'wfacp_incomplete_variation';
							$attributes_keys[]  = ucwords( str_replace( array( 'attribute_', 'attribute_pa_' ), '', $a_key ) );
					}
				}
			}

			return array( $product_attributes, $is_variation_error, $attributes_keys, $variation_attributes );
		}
		 ...
		 * @return mixed|void
		 */
		final public static function get_attribute_html( $cart_item, $cart_item_key, $pro, $switcher_settings, $product_data ) {
		...
		/**
		 * Render inline interactive variation dropdowns for a variable product.
		 *
		 * @param WC_Product $pro            The product (variation or variable).
		 * @param array      $product_data   Product data from switcher.
		 * @param array|null $cart_item      Cart item data if product is in cart.
		 * @param int        $cart_variation_id Currently selected variation ID.
		 *
		 * @return string HTML of inline variation dropdowns.
		 */
		final public static function get_inline_attribute_dropdowns( $pro, $product_data, $cart_item = null, $cart_variation_id = 0 ) {
			...
			if ( in_array( $pro->get_type(), WFACP_Common::get_variation_product_type() ) ) {
				...
				$parent_id = $pro->get_id();
			}

			$product_obj = WFACP_Common::wc_get_product( $parent_id, isset( $product_data['item_key'] ) ? $product_data['item_key'] : '' );
			if ( ! $product_obj instanceof WC_Product || ! $product_obj->is_type( 'variable' ) ) {
				return '';
			}
			...
			// Build attribute labels map
			$attr_labels = array();
			foreach ( $variation_attributes as $slug => $attribute ) {
				$attr_name                = $attribute->get_name();
				$attr_key                 = 'attribute_' . sanitize_title( $attr_name );
				$attr_labels[ $attr_key ] = wc_attribute_label( $attr_name, $product_obj );

				// Build term slug => name map for taxonomy attributes
				if ( $attribute->is_taxonomy() ) {
					$terms = $attribute->get_terms();
					if ( ! is_null( $terms ) ) {
						foreach ( $terms as $term ) {
							if ( ! isset( $attr_labels['_term_names'] ) ) {
								$attr_labels['_term_names'] = array();
							}
							$attr_labels['_term_names'][ $term->slug ] = $term->name;
						}
					}
				}
			}

			$term_names = isset( $attr_labels['_term_names'] ) ? $attr_labels['_term_names'] : array();
			unset( $attr_labels['_term_names'] );

			$item_key = isset( $product_data['item_key'] ) ? $product_data['item_key'] : '';

			$html  = '<div class="wfacp_inline_variations" data-product-id="' . esc_attr( $parent_id ) . '"';
			$html .= ' data-item-key="' . esc_attr( $item_key ) . '"';
			$html .= " data-product_variations='" . esc_attr( wp_json_encode( $available_variations ) ) . "'";
			$html .= '>';

			$html .= '<select class="wfacp_inline_variation_select" name="wfacp_inline_variation_select">';
			...
			foreach ( $available_variations as $variation ) {
				...
				// Build label: "Color: Blue, Size: Large"
				$parts = array();
				foreach ( $attrs as $attr_key => $attr_val ) {
					$label = isset( $attr_labels[ $attr_key ] ) ? $attr_labels[ $attr_key ] : $attr_key;
					$name  = isset( $term_names[ $attr_val ] ) ? $term_names[ $attr_val ] : $attr_val;
					if ( '' === $name ) {
				...
				$option_label = implode( ', ', $parts );
				$is_selected  = ( (int) $cart_variation_id === (int) $vid );

				$html .= '<option value="' . esc_attr( $vid ) . '" ' . selected( $is_selected, true, false ) . '>' . esc_html( $option_label ) . '</option>';
			}
		 ...
		 * @return array
		 */
		final public static function get_product_switcher_item_you_save( $you_save_text, $price_data, $pro, $product_data, $cart_item, $cart_item_key ) {
			$you_save_text_html = '';
			if ( '' !== $you_save_text ) {
				$subscription_tryl   = 0;
			...
			return array( $you_save_text_html, $subscription_product_string );
		}


		final public static function display_not_selected_attribute( $product_data, $pro ) {
		...
		public static function get_builder_localization() {
			...
			$data['design']['preview_step']   = array(
			...
			$data['settings']['product_switching'] = array(
		...
		public static function get_product_switcher_templates() {
			$templates = array(
				...
				'inline'  => array(
					'slug'      => 'inline',
					'path'      => WFACP_TEMPLATE_COMMON . '/product-switcher/inline/product-switcher.php',
					'name'      => __( 'Inline Variations', 'woofunnels-aero-checkout' ),
					'thumbnail' => WFACP_PLUGIN_URL . '/public/template-common/images/defualt_product_switcher.jpg',
				),
			);
		...
		public static function track_events_content_id_options() {
			...
			$events = array(
				array(
					'id'   => '0',
					'name' => __( 'Select content id parameter', 'woofunnels-aero-checkout' ),
				),
				array(
					'id'   => 'product_id',
					'name' => __( 'Product ID', 'woofunnels-aero-checkout' ),
				),
				array(
					'id'   => 'product_sku',
					'name' => __( 'Product Sku', 'woofunnels-aero-checkout' ),
				),
			);
		...
		public static function track_events_options() {
			...
			$events = array(
				array(
					'id'   => 'load',
					'name' => __( 'On Page Load', 'woofunnels-aero-checkout' ),
				),
				array(
					'id'   => 'button',
					'name' => __( 'On Button Click', 'woofunnels-aero-checkout' ),
				),
				array(
					'id'   => 'email',
					'name' => __( 'On Email Capture', 'woofunnels-aero-checkout' ),
				),
			);
		...
		public static function get_default_global_settings() {
			...
			$wfacp_global_checkout = array(
				...
				'wfacp_data' => array(
					'id'    => 'wfacp-global-checkout',
					'class' => 'wfacp_global_checkout',
			...
			$autocomplete     = array(
				...
				'wfacp_data' => array(
					'id'    => 'wfacp-powerpack',
					'class' => 'wfacp_powerpack',
					'title' => __( 'Address Autocomplete', 'woofunnels-aerocheckout-powerpack' ),
				),
			);
				...
				'fieldsets'                   => array(
					...
					'third_step'  => array(
						...
						array(
							...
							'class'       => '',
							'sub_heading' => '',
							'html_fields' => array( 'order_summary' => true ),
							'fields'      => array(
		...
		public static function get_advanced_fields() {
			...
			$field['bwfan_birthday_date'] = array(
				'required'                      => false,
				'default'                       => '',
		...
		public static function show_cart_empty_message() {
			...
			$shop_text = apply_filters( 'wfacp_continue_shopping_text', __( 'Continue shopping', 'woofunnels-aero-checkout' ) );
		...
		public static function get_cart_undo_message() {
			...
			foreach ( $cart_contents as $cart_item_key => $cart_item ) {
				...
				if ( $item_data && $item_data->is_in_stock() && $item_data->has_enough_stock( $cart_item['quantity'] ) ) {
					...
					$removed_notice      = sprintf( __( '%1$s %2$s', 'woocommerce' ), $item_title, $other );
					$removed_notice     .= sprintf( '<a href="javascript:void(0)" class="%s" data-cart_key="%s" data-item_key="%s">%s</a>', esc_attr( $item_class ), esc_attr( $cart_item_key ), esc_attr( $item_key ), $item_icon );
				} else {
					/* Translators: %s Product title. */
  ...
  <path fill-rule="evenodd" clip-rule="evenodd" d="M1 12C1 5.92487 5.92487 1 12 1C18.0751 1 23 5.92487 23 12C23 18.0751 18.0751 23 12 23C5.92487 23 1 18.0751 1 12ZM12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12C21 16.9706 16.9706 21 12 21Z" fill="currentColor"/>
				...
				<div class="wfacp_order_summary_item_delete wfacp_delete_item_wrap">
					<a href="javascript:void(0)" class="wfacp_remove_item_from_order_summary" data-cart_key="<?php echo esc_attr( $cart_item_key ); ?>" data-product_id="<?php echo esc_attr( $product_id ); ?>"><?php echo $item_icon; ?></a>
				</div>
				<?php
			}
		}
		...
		public static function map_meta_value_for_custom_fields( $meta_value, $field ) {
			...
			if ( in_array( $field['type'], array( 'select2', 'multiselect' ), true ) && is_string( $meta_value ) ) {
		...
		public static function get_optional_checkout_fields( $checkout_id ) {
			...
			$excluded_types = array( 'wfacp_html', 'product' );
