...
if ( ! class_exists( 'WFACP_Content_Validator' ) ) {
	#[\AllowDynamicProperties]
	class WFACP_Content_Validator {
		...
		public static function get_default_allowed_meta_keys() {
			...
			return apply_filters( 'wfacp_allowed_import_meta_keys', $keys );
		}
		...
		public static function validate_bricks_content( $elements ) {
			...
			array_walk_recursive(
				...
				function ( &$value ) {
					...
					if ( WFACP_Content_Validator::contains_php_code( $value ) ) {
						$value = '';
					}
					if ( WFACP_Content_Validator::contains_dangerous_tags( $value ) ) {
						$value = wp_kses_post( $value );
					}
				}
			);
		...
		public static function begin_import() {
			$GLOBALS['wfacp_import_in_progress'] = true;
		}

		public static function end_import() {
			$GLOBALS['wfacp_import_in_progress'] = false;
		}
		 ...
		 * URLs. Any URL whose host is not in the image CDN allowlist is stripped and
		 * counted. Caller should abort on count >= 5.
		 *
		...
		public static function pre_http_request_guard( $pre, $args, $url ) {
			if ( empty( $GLOBALS['wfacp_import_in_progress'] ) ) {
				return $pre;
			}
			...
			if ( 'https' !== $scheme ) {
				return new WP_Error( 'wfacp_import_blocked', 'Import blocked: only HTTPS allowed.' );
			}
			if ( ! empty( $userinfo ) ) {
				return new WP_Error( 'wfacp_import_blocked', 'Import blocked: userinfo not allowed in URL.' );
			}
			if ( ! empty( $port ) && 443 !== (int) $port ) {
				return new WP_Error( 'wfacp_import_blocked', 'Import blocked: non-443 port.' );
			}
			...
			if ( ! in_array( $host, $allowed_hosts, true ) ) {
				return new WP_Error( 'wfacp_import_blocked', 'Import blocked: host not in allowlist.' );
			}
