					...
					<li class="wfacp_single_shipping_method">
						<div class="wfacp_single_shipping">
							<div class="wfacp_shipping_radio">
								<?php
								printf( '<input type="radio" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method" %4$s />', $index, sanitize_title( $method->id ), esc_attr( $method->id ), checked( $method->id, $chosen_method, false ) );
								if ( true == $instance->have_shipping_method() ) {
									printf( '<label for="shipping_method_%s_%s">%s</label>', $index, sanitize_title( $method->id ), WFACP_Common::shipping_method_label( $method ) );
								} else {
									printf( '<label for="shipping_method_%s_%s">%s: <span class="wfacp_shipping_cost_no_field_used">%s</span></label>', $index, sanitize_title( $method->id ), WFACP_Common::shipping_method_label( $method ), WFACP_Common::wc_cart_totals_shipping_method_cost( $method ) );
								}
								?>
							</div>
							<?php
