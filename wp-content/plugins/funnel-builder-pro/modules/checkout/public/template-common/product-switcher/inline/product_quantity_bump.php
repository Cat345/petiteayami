...
?>
<?php
// Enqueue inline product switcher JS
wp_enqueue_script( 'wfacp_inline_product_switcher_js', plugin_dir_url( WFACP_PLUGIN_FILE ) . 'assets/js/inline-product-switcher.js', array( 'jquery', 'wfacp_checkout_js' ), WFACP_VERSION_DEV, true );
?>
	<div class="wfacp_anim wfacp_pro_switch wfacp_inline_ps <?php echo $deviceType . ' ' . $ps_setting_wrapper_class; ?> shop_table wfacp-product-switch-panel <?php echo $classes; ?> wfacp_df_ps" cellspacing="0" id="product_switching_field" <?php echo WFACP_Common::get_fragments_attr(); ?> >
			...
			if ( empty( $wfacp_cart ) ) {
				...
				foreach ( $wfacp_cart as $cart_item_key => $cart_item ) {
					...
					if ( $_product && $_product->exists() && $qty > 0 && apply_filters( 'woocommerce_checkout_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
						...
						$row_item_class = apply_filters( 'woocommerce_cart_item_class', 'woocommerce-cart-form__cart-item cart_item wfacp_product_row wfacp_inline_product_row wfacp-selected-product', $cart_item, $cart_item_key );
						?>
									...
									<div class="wfacp_product_switcher_col wfacp_product_switcher_col_2">
										<div class='wfacp_product_switcher_description'>
											...
											<div class="product-name product_name">
												...
												$is_variation_product = in_array( $pro->get_type(), WFACP_Common::get_variation_product_type() );

												if ( $is_variation_product ) {
													$cart_variation_id         = isset( $cart_item['variation_id'] ) ? $cart_item['variation_id'] : 0;
													$parent_product            = wc_get_product( $pro->get_parent_id() );
													$parent_type               = $parent_product ? $parent_product->get_type() : 'variable';
													$product_data_for_dropdown = isset( $cart_item['_wfacp_options'] ) ? $cart_item['_wfacp_options'] : array();
													if ( empty( $product_data_for_dropdown ) ) {
														$product_data_for_dropdown = array( 'type' => $parent_type );
													}
													if ( ! isset( $product_data_for_dropdown['type'] ) ) {
														$product_data_for_dropdown['type'] = $parent_type;
													}
													$inline_dropdowns = WFACP_Common::get_inline_attribute_dropdowns( $pro, $product_data_for_dropdown, $cart_item, $cart_variation_id );
													if ( '' !== $inline_dropdowns ) {
														echo $inline_dropdowns;
													}
												} else {
								...
								<div class="wfacp_product_sec_start">
									...
									<div class="wfacp_product_switcher_col wfacp_product_switcher_col_3 <?php echo $clssaleIndi; ?>">
										...
										<div class="wfacp_product_price_container product-price">
											<div class="wfacp_product_price_sec wfacp_inline_price_sec">
												<?php
