...
if ( ! class_exists( '\FKCart\Pro\Special_Add_On' ) ) {
	...
	class Special_Add_On {
		...
		private function __construct() {
			 ...
			 */
			add_action( 'woocommerce_add_to_cart', array( $this, 'handle_special_addon_product' ), 9999, 2 );
			add_action( 'woocommerce_cart_emptied', array( $this, 'unset_special_addon_product' ) );
			/**
			 * Styling of special Add on
			 */
			add_action( 'wp_footer', array( $this, 'internal_style' ) );
			add_action( 'admin_footer', array( $this, 'internal_style' ) );
			add_filter( 'fkcart_css_var_style', array( $this, 'add_special_addon_css_variables' ) );
			add_action( 'fkcart_after_coupon_section', array( $this, 'special_addon_html' ) );
		}
		...
		public function handle_special_addon_product( $cart_id, $product_id ) {
			...
			try {
				...
				$enable_cart              = ! empty( $settings['enable_cart'] ) ? intval( $settings['enable_cart'] ) : false;
				...
				/**
				 * Check if cart is disabled or addon is removed by toggle
				 */
				if ( false === $enable_cart || ! empty( $is_remove_addons ) ) {
					return;
				}
				...
				remove_action( 'woocommerce_add_to_cart', array( $this, 'handle_special_addon_product' ), 99 );
		 ...
		 */
		public static function special_product_addon( $data = array() ) {
			try {
				if ( ! empty( $data ) ) {
		...
		public function special_addon_html( $cart_settings ) {
			...
			if ( fkcart_is_preview() || ( isset( $cart_settings['enable_special_addon'] ) && wc_string_to_bool( $cart_settings['enable_special_addon'] ) === true ) ) {
				fkcart_get_template_part( 'cart/special-addon-html', '', array(), true, FKCART_PRO_PATH );
			}
		}
