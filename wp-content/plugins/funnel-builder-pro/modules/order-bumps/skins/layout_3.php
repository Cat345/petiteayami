...
if ( $featured_image ) {
	if ( isset( $design_data[ "product_{$product_key}_featured_image_options" ] ) && isset( $design_data[ "product_{$product_key}_featured_image_options" ]['position'] ) ) {
		...
		if ( ! empty( $image_width ) ) {
			...
			$inline_style .= '}</style>';
			echo wp_strip_all_tags( $inline_style );
		}
	}
}
$selected_layout = '';
...
?>
	<div id="wfob_wrap" class="wfob_wrap_start" data-product-title="<?php echo esc_attr( $wc_product->get_name() ); ?>" data-product-price="<?php echo esc_attr( $wc_product->get_price() ); ?>">
		<?php do_action( 'wfob_before_wrapper', $this->get_id(), $wc_product, $product_key, $cart_item_key ); ?>
		<div id="wfob_main_wrapper_start" class="wfob_wrapper" data-wfob-id="<?php echo esc_attr( $this->get_id() ); ?>">
			<div class="wfob_bump wfob_bump_r_outer_wrap wfob_layout_3 <?php echo esc_attr( $image_position_cls ); ?>" data-product-key="<?php echo esc_attr( $product_key ); ?>" data-wfob-id="<?php echo esc_attr( $this->get_id() ); ?>" cart_key="<?php echo esc_attr( $cart_item_key ); ?>">
				<div class="wfob_l3_wrap">
					<div class="wfob_l3_s <?php echo ! $featured_image ? 'wfob_l3_no_img' : ''; ?>">
						...
						<div class="wfob_l3_s_c">
							<div class="wfob_l3_s_data">
								...
								if ( '' != $product_title ) {
									?>
									<div class="wfob_l3_c_head"><?php echo wp_kses_post( $product_title ); ?></div>
									<?php
								}
								if ( '' != $sub_title ) {
									?>
								...
								if ( ( isset( $data['variable'] ) && false == WFOB_Common::display_not_selected_attribute( $data, $wc_product ) ) || true == $allow_choose_options ) {
									printf( "<div class='wfob_l3_c_sub_desc_choose_option'><a href='#' class='wfob_qv-button var_product' qv-id='%d' qv-var-id='%d'>%s</a></div>", $data['id'], $cart_variation_id, apply_filters( 'wfob_choose_option_text', __( 'Choose an option', 'woocommerce' ) ) );
								}
								?>
							</div>
							<div class="wfob_l3_s_btn">
								...
								if ( $enable_price ) {
									...
									<div class="wfob_price">
										<?php echo wp_kses_post( $printed_price ); ?>
									</div>
									<?php
								}
								?>
								<a href="#" class="wfob_l3_f_btn wfob_btn_add <?php echo esc_attr( $checkbox_class ); ?>" style="<?php echo '' !== $cart_item_key ? 'display:none' : ''; ?>"><?php echo wp_kses_post( $add_btn_text ); ?></a>
								<a href="#" class="wfob_l3_f_btn wfob_btn_add wfob_btn_remove <?php echo '' !== $cart_item_key ? 'wfob_item_present' : ''; ?>">
									...
									<span class="wfob_btn_text_remove"><?php echo wp_kses_post( $remove_btn_text ); ?></span>
								</a>
							</div>
					...
					if ( '' !== $description ) {
						...
						?>
						<div class="wfob_l3_s_desc" style="<?php echo esc_attr( $description_display_none ); ?>">
							<div class="wfob_l3_l_desc"><?php echo wp_kses_post( $description ); ?></div>
						</div>
						<?php
					}
					?>
