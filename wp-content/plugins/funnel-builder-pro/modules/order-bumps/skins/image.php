...
if ( isset( $design_data[ 'product_' . $product_key . '_featured_image_options' ] ) ) {
	...
	if ( 'custom' == $image_options['type'] ) {
		if ( ! empty( $image_options['custom_url'] ) ) {
			echo "<img src='" . esc_url( $image_options['custom_url'] ) . "' class='attachment-woocommerce_thumbnail size-woocommerce_thumbnail'>";
		} else {
...
if ( isset( $data['variable'] ) && 'yes' == $data['variable'] && empty( $cart_item_key ) ) {
	...
	echo "<img src='" . esc_url( $image_url ) . "' class='attachment-woocommerce_thumbnail size-woocommerce_thumbnail'>";
} else {
