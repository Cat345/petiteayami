...
$output_response    = isset( $output_response['response'][ $product_key ] ) ? $output_response['response'][ $product_key ] : array();
$wfob_error_message = '';
...
?>
	<div id="wfob_wrap" class="wfob_wrap_start" data-product-title="<?php echo esc_attr( $wc_product->get_name() ); ?>" data-product-price="<?php echo esc_attr( $wc_product->get_price() ); ?>">
		...
		<div class="<?php echo implode( ' ', array( 'wfob_wrapper', 'wfob_' . $selected_layout ) ); ?>" data-wfob-id="<?php echo esc_attr( $this->get_id() ); ?>">
			<div class="wfob_bump wfob_clear <?php echo esc_attr( $image_position_cls ); ?>" data-product-key="<?php echo esc_attr( $product_key ); ?>" data-wfob-id="<?php echo esc_attr( $this->get_id() ); ?>" cart_key="<?php echo esc_attr( $cart_item_key ); ?>">
						...
						<div class="wfob_bgBox_table no_table">
							<div class="wfob_bgBox_tablecell no_table_cell wfob_check_container">
								<div class="wfob_order_wrap wfob_content_bottom_wrap">
									<div class="wfob_bgBox_table_box">
										<div class="wfob_bgBox_cell wfob_img_box">
											<div class="wfob_checkbox_input_wrap">
												...
												if ( wc_string_to_bool( $design_data['header_enable_pointing_arrow'] ) ) {
													...
													?>
													<span class="wfob_blink_img_wrap"><img src="<?php echo esc_url( $blink_url ); ?>"></span>
													<?php
												}
												$disabled = '';
												...
												<input type="checkbox" name="<?php echo esc_attr( $product_key ); ?>" id="<?php echo esc_attr( $product_key ); ?>" data-value="<?php echo esc_attr( $product_key ); ?>" class="wfob_checkbox <?php echo esc_attr( $checkbox_class ); ?>" <?php echo '' != $cart_item_key ? 'checked' : ''; ?> <?php echo esc_attr( $disabled ); ?>>
											</span>
											</div>
										</div>
										<?php
										...
										<div class="wfob_bgBox_cell">
											<div class="wfob_content_sec <?php echo esc_attr( $enable_pointer ); ?>">
												<label for="<?php echo esc_attr( $enable_for_label ); ?>" class="wfob_title wfob_bgBox_cell <?php echo esc_attr( $wfob_merge_tag_in_title ); ?>"> <?php echo wp_kses_post( do_shortcode( $titleHeading ) ); ?> </label>
												<?php
												if ( $enable_price ) {
													?>
									...
									<div class="wfob_error_message"><?php echo esc_html( $wfob_error_message ); ?></div>
								</div>
								<?php if ( $enable_price ) { ?>
