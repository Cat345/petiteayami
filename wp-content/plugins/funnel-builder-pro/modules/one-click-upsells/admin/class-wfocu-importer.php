...
if ( ! class_exists( 'WFOCU_Importer' ) ) {
	...
	class WFOCU_Importer {
		...
		function maybe_import( $posted_data = null ) {
			...
			if ( null === $posted_data ) {
				$posted_data = bwf_clean( $_POST ); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce verified below via wp_verify_nonce on $posted_data['wfocu-action-nonce']
			} else {
				$skip_nonce = true;
			}
			...
			// phpcs:disable WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- $_FILES values are server-generated by PHP's upload mechanism; tmp_name is a system path, name is passed through sanitize_file_name below
			if ( empty( $_FILES['file']['tmp_name'] ) ) {
				wp_die( __( 'Please upload a file to import', 'wfocu' ) );
			}

			$filename = sanitize_file_name( isset( $_FILES['file']['name'] ) ? $_FILES['file']['name'] : '' );
			...
			if ( isset( $_FILES['file']['size'] ) && $_FILES['file']['size'] > 5 * 1024 * 1024 ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- server-generated integer
				wp_die( __( 'Import file is too large. Maximum allowed size is 5 MB.', 'wfocu' ) );
			}

			// Retrieve the settings from the file and convert the JSON object to an array.
			$funnels = json_decode( file_get_contents( $file ), true ); //phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
				...
				foreach ( $funnels as $funnel ) {
					...
					$funnel_to_create = array(
						...
						'meta'        => array(
							...
							'_wfocu_rules'            => isset( $funnel['_wfocu_rules'] ) ? $funnel['_wfocu_rules'] : array(),
							'_funnel_steps'           => array(),
