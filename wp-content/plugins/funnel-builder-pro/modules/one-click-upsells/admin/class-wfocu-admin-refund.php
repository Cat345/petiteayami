...
if ( ! class_exists( 'WFOCU_Admin_Refund' ) ) {
	...
	class WFOCU_Admin_Refund {
		...
		public function wfocu_register_admin_offer_refund_meta_boxes() {
			...
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Admin order page parameter detection
			if ( isset( $_GET['id'] ) ) {
				$this->order_id = absint( wp_unslash( $_GET['id'] ) );//phpcs:ignore WordPress.Security.NonceVerification.Recommended
			}
			 ...
			 */
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Complex query with table prefixes and joins, order_id sanitized with absint
			$new_order_query = $wpdb->prepare(
				"SELECT COUNT(event.id) FROM `{$wpdb->prefix}wfocu_event` as event LEFT JOIN {$wpdb->prefix}wfocu_event_meta as metatable ON ( event.id = metatable.event_id AND metatable.meta_key = '_new_order') INNER JOIN {$wpdb->prefix}wfocu_session as session ON (event.sess_id = session.id) WHERE session.order_id = %d AND event.action_type_id = 4 AND (metatable.event_id IS NULL)",
				$this->order_id
			);

			$new_order_count = (int) $wpdb->get_var( $new_order_query ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared ,db call ok; no-cache ok; WPCS: unprepared SQL ok.,
			...
			if ( 0 < $new_order_count ) {
				...
				add_meta_box(
					...
					__( 'One Click Upsells Refund', 'woofunnels-upstroke-one-click-upsell' ),
					array(
						$this,
		...
		public function wfocu_move_offer_refund_metabox( $already ) {
			...
			if ( ! isset( $already['normal'] ) ) {
				$normal = array( 'wfocu-offer-refund-metabox' );
			} else {
