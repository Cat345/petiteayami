<div v-if="product.vars_count>0" class="have_variation">
	<?php _e( "{{product.vars_count}} Variants <a class='have_variation_expand'>(Expand</a> / <a class='have_variation_close'>Close)</a> ", 'woofunnels-upstroke-one-click-upsell' ); // phpcs:ignore WordPress.Security.EscapeOutput ?>
</div>
<div v-if="product.vars_count>0 && isEmpty(allselectedVars[index])" class="have_variation red_notice">
	<?php esc_html_e( 'In case of no variants selected, offer will skip during funnel.', 'woofunnels-upstroke-one-click-upsell' ); ?>
</div>
<table width="100%" class="variation_products" id="variant_product_id" v-bind:data-index="index" v-if="product.vars_count>0" border="1">
	...
	<tr>
		...
		<th><?php esc_html_e( 'Default', 'woofunnels-upstroke-one-click-upsell' ); ?></th>
		<th><?php esc_html_e( 'Attributes', 'woofunnels-upstroke-one-click-upsell' ); ?></th>
		<th><?php esc_html_e( 'Price', 'woofunnels-upstroke-one-click-upsell' ); ?></th>
	...
	<tr v-for="(variation, var_index) in product.variations" v-bind:id="variation.vid" class="product_variation_row">
		...
		<td>
			<div class=" product_options">
				<p v-if="variation.regular_price!=='undefined'"><?php esc_html_e( 'Regular Price', 'woofunnels-upstroke-one-click-upsell' ); ?>: <span v-html="variation.regular_price"></span>
				</p>

				<br/>
				<p v-if="variation.hasOwnProperty('price') && variation.price != ''"><?php esc_html_e( 'Sale Price', 'woofunnels-upstroke-one-click-upsell' ); ?>: <span v-html="variation.price"></span></p>
				<br v-if="variation.hasOwnProperty('price') && variation.price != ''"/>
				<p><?php esc_html_e( 'Offer Price', 'woofunnels-upstroke-one-click-upsell' ); ?>: <span v-bind:class="'wfocu_of_price_var_'+index+'_'+var_index" v-html="offer_price_html_var(variation,product)"></span>
