	...
	class WFOCU_Gateway_Integration_Mollie_Gateway_Abstract extends WFOCU_Gateway {
		...
		public function process_charge( $order ) {
			if ( 'mollie_wc_gateway_creditcard' === $this->get_key() || true === $this->is_sepa_enabled() ) {
				...
				$order_behavior = WFOCU_Core()->funnels->get_funnel_option( 'order_behavior' );
				$is_batching_on = ( 'batching' === $order_behavior ) ? true : false;
				$totals         = isset( $post_data['totals'] ) ? $post_data['totals'] : array();
		...
		public function generate_mollie_charge( $new_order, $order ) {
			...
			try {
				...
				// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Exception message is internal
				throw new WFOCU_Payment_Gateway_Exception( sprintf( esc_html__( 'The customer (%1$s) could not be used or found in %2$s. ', 'upstroke-woocommerce-one-click-upsell-mollie' ), esc_html( $customer_id ), esc_html( $this->key ) ), 101, $this->get_key() );
			}
			...
			try {
				if ( $validMandate ) {
					...
					// phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Exception message is internal
					throw new WFOCU_Payment_Gateway_Exception( sprintf( esc_html__( 'The customer (%1$s) does not have a valid mandate in %2$s. ', 'upstroke-woocommerce-one-click-upsell-mollie' ), esc_html( $customer_id ), esc_html( $this->key ) ), 101, $this->get_key() );
				}
			} catch ( Mollie\Api\Exceptions\ApiException $e ) {
		...
		public function onBatchWebhookAction() {
			...
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Mollie callback parameter
			$order_id = sanitize_text_field( wp_unslash( $_GET['order_id'] ) );
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Mollie callback parameter
			$key = sanitize_text_field( wp_unslash( $_GET['key'] ) );
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Mollie callback parameter
			$item_key = sanitize_text_field( wp_unslash( $_GET['item_key'] ) );
		...
		public function process_refund_offer( $order ) {
			...
			try {
				...
				$refund = WFOCU_Mollie_Helper_Compat::get_api_client( WFOCU_Mollie_Helper::instance()->container, $test_mode )->payments->refund(
					$payment_object,
					array(
						'amount'      => array(
