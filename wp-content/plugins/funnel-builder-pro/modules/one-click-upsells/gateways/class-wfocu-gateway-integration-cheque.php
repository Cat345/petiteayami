...
if ( ! class_exists( 'WFOCU_Gateway_Integration_Cheque' ) ) {
	...
	class WFOCU_Gateway_Integration_Cheque extends WFOCU_Gateway {
		...
		public function __construct() {
			...
			/**
			 * Hook into Cheque payment processing to setup upsell
			 */
			add_filter( 'woocommerce_cheque_process_payment_order_status', array( $this, 'maybe_setup_upsell_for_cheque' ), 999, 2 );
		...
		/**
		 * Setup upsell for Cheque payments and set primary order status
		 *
		 ...
		 * @return string
		 */
		public function maybe_setup_upsell_for_cheque( $order_status, $order ) {
			...
			remove_filter( 'woocommerce_payment_complete_order_status', array( WFOCU_Core()->orders, 'maybe_set_completed_order_status' ), 999 );
			...
			if ( WFOCU_Core()->data->is_funnel_exists() && $get_integration instanceof WFOCU_Gateway && $get_integration->is_enabled( $order ) && $get_integration->has_token( $order ) ) {
				...
				WFOCU_Core()->public->upsell_skip_reason( $wc_get_order, $get_integration );
