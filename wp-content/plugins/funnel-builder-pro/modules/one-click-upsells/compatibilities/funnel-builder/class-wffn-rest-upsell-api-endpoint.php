...
if ( ! class_exists( 'WFFN_REST_UPSELL_API_EndPoint' ) ) {
	class WFFN_REST_UPSELL_API_EndPoint extends WFFN_REST_Controller {
		...
		public function register_endpoint() {
			...
			register_rest_route(
				$this->namespace,
				'/' . 'funnel-upsell' . '/(?P<step_id>[\d]+)' . '/offers',
				array(
					'args'   => array(
						'step_id' => array(
			...
			register_rest_route(
				$this->namespace,
				'/' . 'funnel-offer' . '/(?P<step_id>[\d]+)' . '/products',
				array(
					'args'   => array(
						'step_id' => array(
							'description' => __( 'Current step id.', 'funnel-builder-powerpack' ),
			...
			register_rest_route(
				$this->namespace,
				'/' . 'funnel-offer' . '/(?P<step_id>[\d]+)' . '/products' . '/save-layout',
				array(
					'args'   => array(
						'step_id' => array(
							'description' => __( 'Current step id.', 'funnel-builder-powerpack' ),
			...
			register_rest_route(
				$this->namespace,
				'/' . 'funnel-upsell' . '/(?P<step_id>[\d]+)' . '/rules',
				array(
					'args'   => array(
						'step_id' => array(
							'description' => __( 'Current step id.', 'funnel-builder-powerpack' ),
			...
			register_rest_route(
				$this->namespace,
				'/' . 'funnel-upsell' . '/(?P<step_id>[\d]+)' . '/dynamic-path',
				array(
					'args'   => array(
						'step_id' => array(
							'description' => __( 'Current step id.', 'funnel-builder-powerpack' ),
		...
		public function wfocu_remove_product( WP_REST_Request $request ) {
			...
			if ( absint( $offer_id ) > 0 && ! empty( $options ) ) {
				...
				$product_key = ! empty( $options['product_key'] ) ? bwf_clean( $options['product_key'] ) : '';
		...
		public function wfocu_save_offer_products( WP_REST_Request $request ) {
			...
			if ( absint( $offer_id ) > 0 && ! empty( $options ) ) {
				...
				if ( $step_id > 0 && $offer_id > 0 ) {
					...
					$post = get_post( bwf_clean( wp_unslash( $step_id ) ) );  // input var ok
					if ( ! is_null( $post ) ) {

						$funnel_id   = $post->ID;
						$offer_id    = isset( $offer_id ) ? bwf_clean( wp_unslash( $offer_id ) ) : 0; // input var ok
						$offers      = ( isset( $options['offers'] ) && is_array( bwf_clean( wp_unslash( $options['offers'] ) ) ) && count( bwf_clean( wp_unslash( $options['offers'] ) ) ) ) > 0 ? bwf_clean( wp_unslash( $options['offers'] ) ) : WFOCU_Core()->funnels->get_funnel_steps( $funnel_id ); // input var ok
		...
		public function get_on_jump_offer( $step_id, $offer_id ) {
			...
			if ( is_array( $offers ) && count( $offers ) ) {
				...
				$drop_option2                       = array();
				$drop_option2['Terminate Funnel'][] = array(
					'label' => __( 'Thank You Page', 'funel-builder' ),
					'value' => 'terminate',
					'key'   => 'terminate',
				);
				...
				if ( count( $jump_offers ) ) {
					...
					$drop_option1['select'][] = array(
						'label' => __( 'Select an offer', 'funnel-builder-powerpack' ),
						'value' => 'automatic',
						'key'   => 'automatic',
					);
		...
		public function wfocu_add_product( WP_REST_Request $request ) {
			...
			if ( absint( $offer_id ) > 0 && ! empty( $options ) ) {
				...
				$fields        = array();
				$variations    = array();
				$products_list = ( isset( $product_data ) && is_array( bwf_clean( $product_data ) ) && count( bwf_clean( $product_data ) ) > 0 ) ? bwf_clean( $product_data ) : array();
			...
			foreach ( $wfocu_rules as $k => $rule_cats ) {
				...
				foreach ( $rule_cats as $grpk => $grp ) {
					...
					foreach ( $grp as $rule_key => $rule ) {
						if ( ! in_array( $rule['rule_type'], array( 'general_always_2', 'general_always' ), true ) ) {
							if ( ! isset( $new_rules[ $k ][ $grpk ] ) ) {
								$new_rules[ $k ][ $grpk ] = array();
							}
				...
				if ( ! empty( $rule_set ) ) {
					...
					foreach ( $rule_set as $rule ) {
						...
						if ( in_array( $rule_type, array( 'Cart_Product_Select', 'Product_Select' ), true ) ) {
							$products            = ( ! empty( $saved_rules ) && isset( $saved_rules['product'] ) && is_array( $saved_rules['product'] ) ) ? $this->get_product_from_conditions( $saved_rules['product'] ) : array();
							$products_from_basic = ( ! empty( $saved_rules ) && isset( $saved_rules['basic'] ) && is_array( $saved_rules['basic'] ) ) ? $this->get_product_from_conditions( $saved_rules['basic'] ) : array();

							$values = array_merge( $products, $products_from_basic );
						}
						if ( in_array( $rule_type, array( 'Coupon_Select' ), true ) ) {
							$coupons = ( ! empty( $saved_rules ) && is_array( $saved_rules ) ) ? $this->get_coupons_from_conditions( $saved_rules['basic'] ) : array();
		...
		public function get_wfocu_offer_details( WP_REST_Request $request ) {
			...
			if ( class_exists( 'WFOCU_Common' ) && absint( $offer_id ) > 0 && absint( $upsell_id ) > 0 ) {
				...
				if ( empty( $offer ) ) {
					$offer               = new stdClass();
					$offer->settings     = array();
					$offer->ship_dynamic = array();
					$offer->InitialValue = array();
					$offer->fields       = array();
					$offer->products     = array();
					$offer->variations   = array();
				}

				$_REQUEST['offer_id']         = $offer_id; //phpcs:ignore WordPress.Security.NonceVerification.Missing , FunnelBuilder.CodeAnalysis.FunnelBuilderSpecific.MissingCapabilityCheck
						...
						if ( $product instanceof WC_Product ) {
							...
							if ( $product->is_type( 'variable' ) ) {
								...
								if ( ! empty( $available_variations ) ) {
									$product_variations = isset( $offer_variations[ $key ] ) ? $this->get_formatted_variations( $available_variations, $offer_variations[ $key ] ) : array();
								}
							}
							...
							if ( ! empty( $offer_schemes ) ) {
								$selected_schemes = ( isset( $offer->schemes->{$key} ) && is_array( $offer->schemes->{$key} ) ) ? array_keys( $offer->schemes->{$key} ) : array();
								...
								if ( ! empty( $offer->schemes->{$key} ) ) {
									...
									$offer_schemes = array_map(
										function ( $scheme ) use ( $save_schemes ) {
				...
				if ( ! empty( $offer->settings ) ) {
					...
					$offer_values->jump_on_accepted             = ( ! empty( $settings->jump_on_accepted ) && true === wc_string_to_bool( $settings->jump_on_accepted ) ) ? (array) 'true' : array();
					$offer_values->jump_on_rejected             = ( ! empty( $settings->jump_on_rejected ) && true === wc_string_to_bool( $settings->jump_on_rejected ) ) ? (array) 'true' : array();
					...
					$offer_values->subscription_discount        = ( ! empty( $settings->subscription_discount ) && true === wc_string_to_bool( $settings->subscription_discount ) ) ? (array) 'true' : array();
					$offer_values->subscription_signup_discount = ( ! empty( $settings->subscription_signup_discount ) && true === wc_string_to_bool( $settings->subscription_signup_discount ) ) ? (array) 'true' : array();
					...
					$offer_values->is_override_free_trial       = ( ! empty( $settings->is_override_free_trial ) && true === wc_string_to_bool( $settings->is_override_free_trial ) ) ? (array) 'true' : array();
					$offer_values->qty_selector                 = ( ! empty( $settings->qty_selector ) && true === wc_string_to_bool( $settings->qty_selector ) ) ? (array) 'true' : array();
		...
		public function get_offer_fields( $upsell_id, $offer_id ) {
			...
			$offer_settings_fields[] = array(
				'type'   => 'checklist',
				'key'    => 'ship_dynamic',
				'label'  => __( 'Dynamic Shipping', 'funnel-builder-powerpack' ),
				'hint'   => '',
				'values' => array(
					0 => array(
						'value' => 'true',
						'name'  => __( 'Check this box to charge the user separately for the shipping of this item. The cost will be calculated on the fly based on your store\'s configuration and shown to the user upon clicking ‘accept’. Any Flat Shipping charges set above will be overridden by dynamic shipping.', 'funnel-builder-powerpack' ),
					),
				),
			);
		...
		public function wfocu_add_offer( WP_REST_Request $request ) {
			...
			if ( ! empty( $posted_data ) && absint( $upsell_id ) > 0 ) {  // Input var okay.
				...
				if ( $inherit_id > 0 && '' !== $title ) {
					...
					$upsell_id = bwf_clean( $upsell_id );  // Input var okay.
					...
					$post      = array(
						'post_title'  => bwf_clean( wp_unslash( $title ) ), // Input var okay.
						'post_type'   => $post_type,
						'post_status' => 'publish',
					);
					...
					if ( ! is_wp_error( $new_offer_id ) ) {
						...
						$default_settings = array(
							...
							'name'  => bwf_clean( wp_unslash( $title ) ),
							'state' => 1,
							'id'    => $new_offer_id,
						);
				...
				if ( ! empty( $new_offer_id ) && absint( $new_offer_id ) > 0 ) {
					...
					if ( ! empty( $canvas_data ) ) {
						...
						$offer['substeps']      = array( 'offer' => array( $offer ) );
			...
			if ( $canvas_data['empty'] !== true ) {
				if ( ! empty( $canvas_data['offer_id'] ) ) {
					...
					if ( $insert_position !== false && ( $canvas_data['position'] === 'accept' || $canvas_data['position'] === 'reject' || $canvas_data['position'] !== 'end' ) ) {
						...
						array_splice( $steps, $insert_index, 0, array( $lastElement ) );
						update_post_meta( $upsell_id, '_funnel_steps', $steps );
					}
		...
		public function update_product_schema( $posted_product ) {
			...
			if ( $product instanceof WC_Product ) {
				...
				if ( $product->is_type( 'variable' ) ) {
					$available_variations = $product->get_children();
					if ( ! empty( $available_variations ) ) {
						$product_variations = $this->get_formatted_variations( $available_variations, array() );
