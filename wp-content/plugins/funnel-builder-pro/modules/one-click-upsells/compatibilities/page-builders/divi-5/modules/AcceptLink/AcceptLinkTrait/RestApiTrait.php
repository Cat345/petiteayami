...
trait RestApiTrait {
	...
	/**
	 * Register REST API endpoints for Accept Link module.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	...
	/**
	 * REST API callback to get products for Accept Link.
	 *
	 * @since 1.0.0
	...
	register_rest_route(
		...
		array(
			...
			'callback'            => function ( \WP_REST_Request $request ) {
				$class_name = 'WFOCU\Modules\AcceptLink\AcceptLink';
				if ( class_exists( $class_name ) && method_exists( $class_name, 'get_products' ) ) {
					return $class_name::get_products( $request );
				}
				return new \WP_Error( 'class_not_found', 'AcceptLink class not found', array( 'status' => 500 ) );
			},
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
