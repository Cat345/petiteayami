...
trait RestApiTrait {
	...
	/**
	 * Register REST API endpoints for Qty Selector module.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	...
	/**
	 * REST API callback to get products for Qty Selector.
	 *
	 * @since 1.0.0
	...
	public static function get_products( \WP_REST_Request $request ) {
		...
		if ( ! isset( $template_loader->product_data->products ) || empty( (array) $template_loader->product_data->products ) ) {
			$offer_id = self::get_offer_id( $request );
			if ( ! empty( $offer_id ) ) {
	...
	/**
	 * Get offer ID from request, referer URL, or template loader.
	 *
	 * @since 1.0.0
	 * @param \WP_REST_Request $request REST request object.
	...
	private static function get_offer_id( \WP_REST_Request $request = null ): int {
		...
		if ( $request ) {
			...
			if ( ! empty( $post_id ) ) {
				...
				// Check if this post_id is a wfocu_offer post type
				$post = get_post( $post_id );
				if ( $post && $post->post_type === 'wfocu_offer' ) {
					$offer_id = $post_id;
				}
			}
		}
		...
		if ( empty( $offer_id ) && ! empty( $_SERVER['HTTP_REFERER'] ) ) {
	...
	register_rest_route(
		...
		array(
			...
			'callback'            => function ( \WP_REST_Request $request ) {
				$class_name = 'WFOCU\Modules\QtySelector\QtySelector';
				if ( class_exists( $class_name ) && method_exists( $class_name, 'get_products' ) ) {
					return $class_name::get_products( $request );
				}
				return new \WP_Error( 'class_not_found', 'QtySelector class not found', array( 'status' => 500 ) );
			},
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
