...
if ( ! class_exists( 'WFOCU_Template_Group_Divi5' ) ) {
	...
	class WFOCU_Template_Group_Divi5 extends WFOCU_Template_Group {
		...
		public function __construct( $slug = 'divi' ) {
			parent::__construct();
			$this->slug = $slug;
		}
		...
		protected function get_template_divi() {
			return plugin_dir_path( WFOCU_PLUGIN_FILE ) . 'compatibilities/page-builders/divi-5/class-wfocu-template-divi5.php';
		}
		...
		public function update_template( $template, $offer, $offer_settings ) {
			...
			if ( true === $response['found'] && empty( $response['error'] ) ) {
				...
				require_once plugin_dir_path( WFOCU_PLUGIN_FILE ) . 'compatibilities/page-builders/divi-5/class-wfocu-divi5-importer.php';

				$obj = new WFOCU_Divi5_Importer();
	...
	WFOCU_Core()->template_loader->register_group( new WFOCU_Template_Group_Divi5( 'divi' ), 'divi' );
	WFOCU_Core()->template_loader->register_group( new WFOCU_Template_Group_Divi5( 'divi5' ), 'divi5' );
}
