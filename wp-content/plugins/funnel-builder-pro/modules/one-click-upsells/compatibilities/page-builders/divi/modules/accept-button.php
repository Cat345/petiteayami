...
if ( ! class_exists( 'WFOCU_Accept_Button' ) ) {
	...
	class WFOCU_Accept_Button extends WFOCU_Divi_HTML_BLOCK {
		...
		public function setup_data() {
			...
			// Icon Library Field
			$this->add_icon( $tab_id, 'icon', __( 'Icon', 'woofunnels-upstroke-one-click-upsell' ) );
		...
		public function style_field() {
			...
			$this->add_typography( $tab_id, $key . 'title_typography', '%%order_class%% .wfocu-button-wrapper .wfocu-button-content-wrapper span:not(.wfocu-button-icon)', '', $default, array(), $font_side_default );
			...
			$this->add_typography( $tab_id, $key . 'subtitle_typography', '%%order_class%% .wfocu-button-wrapper .wfocu-button-subtitle', __( 'Subtitle Typography', 'woofunnels-upstroke-one-click-upsell' ), '', array(), $font_side_default );
			...
			$this->add_border( $border_id, $key . '_border', '%%order_class%% .wfocu-button-wrapper a.wfocu_upsell', array(), $default_args );
		...
		public function html( $attrs, $content = null, $render_slug = '' ) {
			...
			<div class="wfocu-button-wrapper">
				<a id="wfocu-accept-button-link" class='wfocu_upsell wfocu-accept-button-link wfocu_paypal_in_context_btn' href="javascript:void(0);" data-key="<?php echo $product_key; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" <?php WFOCU_Core()->template_loader->add_attributes_to_buy_button(); ?>>
