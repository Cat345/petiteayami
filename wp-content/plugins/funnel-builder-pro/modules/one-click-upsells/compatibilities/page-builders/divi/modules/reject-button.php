...
if ( ! class_exists( 'WFOCU_Reject_Button' ) ) {
	...
	class WFOCU_Reject_Button extends WFOCU_Divi_HTML_BLOCK {
		...
		private function style_field() {
			...
			$this->add_typography( $tab_id, $key . '_typography', '%%order_class%% .wfocu-reject-button-wrap .wfocu-wfocu-reject', '', '', array(), $font_side_default );
			...
			$this->add_border( $border_id, $key . '_border', '%%order_class%% .wfocu-reject-button-wrap a.wfocu-wfocu-reject', array(), $default_args );
			$this->add_box_shadow(
				$border_id,
				$key . '_box_shadow',
				'%%order_class%% .wfocu-reject-button-wrap .wfocu-wfocu-reject',
				array(
					'enable'   => 'off',
					'vertical' => 0,
					'color'    => '#00B211',
			...
			<div class="wfocu-button-wrapper wfocu-reject-button-wrap">
				<a class="wfocu-wfocu-reject wfocu_skip_offer" href="javascript:void(0);">
					...
					if ( 'left' == $this->props['icon_align'] && '' !== $this->props['icon'] ) {
						?>
						<span class='wfocu-button-icon et-pb-icon'><?php echo html_entity_decode( et_pb_process_font_icon( $this->props['icon'] ), ENT_QUOTES | ENT_HTML401 ); ?></span>
						<?php
					}
