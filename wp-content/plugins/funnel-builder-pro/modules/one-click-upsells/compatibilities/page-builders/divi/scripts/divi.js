(function ($) {
    let create_heading_timeout = null;
    window.wfocu_prepare_divi_css = function (data, utils, props) {
        ...
        for (let t in data.typography_data) {
            (function (key, selector) {
                ...
                main_output.push({'selector': selector, 'declaration': utils.setElementFont(props[property],true)});
    ...
    $(document.body).on('keypress', '.wfocu_divi_border textarea', function (e) {        // IE
        var keynum
        if (window.event) {
            keynum = e.keyCode;
        } else if (e.which) {
    ...
    $(document.body).on('click', '.et-fb-form__toggle', function () {
        ...
        setTimeout((el) => {
            ...
            siblings.each(function () {
                ...
                let subheading = $(this).find('.wfocu_subheading_divi_builder');
