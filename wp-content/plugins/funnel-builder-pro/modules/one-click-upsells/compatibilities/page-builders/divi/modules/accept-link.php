...
if ( ! class_exists( 'WFOCU_Accept_Link' ) ) {
	class WFOCU_Accept_Link extends WFOCU_Divi_HTML_BLOCK {
		...
		private function style_field() {
			...
			$this->add_border( $border_id, 'border', '%%order_class%% .wfocu-wfocu-accept', array(), $default );
			...
			$this->add_box_shadow(
				$border_id,
				'box_shadow',
				'%%order_class%% .wfocu-wfocu-accept',
				array(
					'enable'   => 'off',
					'vertical' => 0,
					'color'    => '#00B211',
		...
		public function html( $attrs, $content = null, $render_slug = '' ) {
			...
			<div class="wfocu-button-wrapper">
				<a class="wfocu-wfocu-accept wfocu_upsell wfocu_paypal_in_context_btn" href="javascript:void(0);" data-key="<?php echo esc_attr( $product_key ); ?>"><?php echo esc_html( $this->props['text'] ); ?></a>
			</div>
			<?php
			return ob_get_clean();
		}
