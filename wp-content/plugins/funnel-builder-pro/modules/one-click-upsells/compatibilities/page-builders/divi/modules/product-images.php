...
if ( ! class_exists( 'WFOCU_Product_Image' ) ) {
	class WFOCU_Product_Image extends WFOCU_Divi_HTML_BLOCK {
		...
		public function setup_data() {
			...
			$this->add_switcher( $tab_id, 'slider_enabled', __( 'Enable Slider', 'woofunnels-upstroke-one-click-upsell' ), 'on', array(), '<i>Note: The Slider will show on frontend only</i>' );
		...
		public function style_field() {
			...
			$this->add_width( $tab_id, 'width', '%%order_class%%  .wfocu-product-gallery .wfocu-carousel-cell > a > img', '', '', array(), true );
			// Need Max Width
		...
		public function html( $attrs, $content = null, $render_slug = '' ) {
			...
			ob_start();
			if ( wp_doing_ajax() ) {
				wp_enqueue_style( 'flickity-css', esc_url( plugin_dir_url( WFOCU_PLUGIN_FILE ) . 'assets/flickity/flickity.css' ), array(), WFOCU_VERSION );
				wp_enqueue_style( 'flickity-common-css', esc_url( plugin_dir_url( WFOCU_PLUGIN_FILE ) . 'assets/css/flickity-common.css' ), array(), WFOCU_VERSION );
			}
