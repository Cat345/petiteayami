...
if ( ! class_exists( 'BWFABT_REST_Variant' ) ) {
	...
	class BWFABT_REST_Variant extends WP_REST_Controller {
		...
		public function register_routes() {
			...
			register_rest_route(
				$this->namespace,
				'/' . $this->rest_base . '/variant/(?P<variant_id>[\d]+)',
				array(
					'args' => array(
						'variant_id' => array(
			...
			register_rest_route(
				$this->namespace,
				'/' . $this->rest_base . '/draft/(?P<variant_id>[\d]+)',
				array(
					array(
						'methods'             => WP_REST_Server::CREATABLE,
			...
			register_rest_route(
				$this->namespace,
				'/' . $this->rest_base . '/publish/(?P<variant_id>[\d]+)',
				array(
					array(
						'methods'             => WP_REST_Server::CREATABLE,
			...
			if ( $variant_id > 0 ) {
				...
				if ( $variantid > 0 ) {
					$resp['data'] = array(
						...
						'title'       => html_entity_decode( $get_controller->get_variant_title( $variant->get_id() ), ENT_QUOTES | ENT_HTML401 ),
						'desc'        => '',
						'traffic'     => '0.00',
			...
			if ( $variant_id > 0 && $experiment_id > 0 ) {
				...
				if ( ! empty( $type ) ) {
					...
					if ( $deleted && count( $active_variants ) < 2 ) {
						...
						$updated_id = $control_varnt->save( array() );
