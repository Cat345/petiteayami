...
if ( ! class_exists( 'WFOCU_Sublium_Compatibility' ) ) {
	class WFOCU_Sublium_Compatibility {
		...
		public function __construct() {
			...
			// Frontend hooks
			add_action( 'wfocu_add_custom_html_above_accept_button', [ $this, 'schemes_template_html' ], 20, 2 );
			add_action( 'footer_after_print_scripts', [ $this, 'render_js' ] );

			// Register shortcode for Sublium plans selector
			add_shortcode( 'wfocu_sublium_plans_selector', [ $this, 'wfocu_sublium_plans_selector_output' ] );

			// Add shortcode to backend list for visibility
			add_filter( 'wfocu_shortcode_list', [ $this, 'add_sublium_plans_selector_to_list' ], 10, 1 );

			// Admin hooks
			add_filter( 'wfocu_rule_type_product_args', [ $this, 'register_rule_type' ], 10, 1 );
			add_filter( 'wfocu_params_localize_script_data', [ $this, 'register_subs_product_search_nonce' ] );
			add_action( 'wp_ajax_wfocu_subs_product_search', [ $this, 'subs_product_search' ] );

			// Data processing hooks
			add_filter( 'wfocu_offer_data', [ $this, 'add_scheme_plan_data' ], 10, 2 );
			add_filter( 'sublium_wcs_plan_data', [ $this, 'attach_discounting_data' ], 10, 2 );
			add_filter( 'sublium_wcs_subscription_price', [ $this, 'set_subscription_prices_for_calculation' ], 10, 3 );
			add_filter( 'wfocu_offer_after_product_added', [ $this, 'add_scheme_product_level' ], 10, 4 );

			// Validation hook to prevent upsell when COD is selected with Sublium subscription products
			add_filter( 'wfocu_offer_validation_result', [ $this, 'maybe_validate_sublium_cod' ], 10, 2 );
			add_filter( 'wfocu_variations_attributes', [ $this, 'modify_variations_attributes' ], 10, 4 );
			add_action('wfocu_offer_setup_completed', [ $this, 'clear_elementor_cache' ], 5, 2 );

			// Add has_sublium_plans key to product data in API responses
			add_filter( 'wfocu_offer_product_details', [ $this, 'add_sublium_plans_data' ], 10, 5 );
			add_filter( 'wfocu_offer_product_schema', [ $this, 'add_sublium_plans_data_schema' ], 10, 3 );
		}
	...
	public function modify_variations_attributes( $attributes, $variation, $product ,$discount_data) {
		...
		public function attach_discounting_data( $data, $plan ) {
			...
			if ( ! empty( $upsell_discount_data ) ) {
				...
				$data['_sublium_data'] = [ 'upsell_discount_data' => $upsell_discount_data ];
			}
	...
	private function render_plan_options( $product_id, $product_key ) {
		...
		$plans_json=array_map(function($plan) use ($product){
		...
		add_action('wp_footer', function() use ($inline_script,$script_handle){
	...
	private function is_one_time_purchase_enabled() {
		...
		public function get_subscription_products_options( $product_id, $offer_data = '', $product_key = '', $is_front = false ) {
			...
			return [ 'plans' => $this->get_cached_plans(absint($product_id)), 'discount_data' => $discount_data ];
		...
		public function subs_product_search( $str = '' ) {
			...
			// Get search term
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- This is a public search endpoint
			$term          = empty( $str ) ? ( isset( $_REQUEST['term'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['term'] ) ) : '' ) : $str;
